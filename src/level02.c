...
int mx_level02_call(SDL_Renderer *renderer, SDL_Window *window, int level_number, int is_paused) {
    ...
    if (DEV_MODE)
        level.mapTexture = mx_load_texture("resource/resources/levels/level-2/dev-mod.png", renderer);
    else level.mapTexture = mx_load_texture("resource/resources/levels/level-2/bg.png", renderer);
    level.doorTexture = mx_load_texture("resource/resources/door.png", renderer);
    level.number = 2;
    level.puzzle_number = 0;
    mx_read_doors("resource/resources/levels/level-2/level.info", &level);
    // Загрузка коллизии
    if (mx_load_collision(renderer, &level, level_number))
        return EXIT;

    t_man man;
    mx_init_man(&man, 100, 100, &level, 1);
