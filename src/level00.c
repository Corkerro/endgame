...
int mx_level00_call(SDL_Renderer *renderer, SDL_Window *window, int level_number, int is_paused) {
    ...
    else level.mapTexture = mx_load_texture("resource/resources/levels/level-0/bg.png", renderer);
    level.doorTexture = mx_load_texture("resource/resources/door.png", renderer);
    level.number = 0;
    level.puzzle_number = 0;
    mx_read_doors("resource/resources/levels/level-0/level.info", &level);
    // Загрузка коллизии
