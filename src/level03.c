...
int mx_level03_call(SDL_Renderer *renderer, SDL_Window *window, int level_number, int is_paused) {
    ...
    if (DEV_MODE)
        level.mapTexture = mx_load_texture("resource/resources/levels/level-3/dev-mod.png", renderer);
    else level.mapTexture = mx_load_texture("resource/resources/levels/level-3/bg.png", renderer);
    level.doorTexture = mx_load_texture("resource/resources/door.png", renderer);
    level.number = 3;
    level.puzzle_number = 0;
    mx_read_doors("resource/resources/levels/level-3/level.info", &level);
    // Загрузка коллизии
